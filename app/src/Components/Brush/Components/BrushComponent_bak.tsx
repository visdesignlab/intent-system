...
import SingleBrushComponent from './SingleBrushComponent_bak';
...
const BrushComponent: FC<Props> = ({
  ...
  useEffect(() => {
    ...
    if (initialBrushes) {
      setStateEff({ ...state, brushes: initialBrushes });
    }
  });
  ...
  function handleMove(event: MouseEvent) {
    if (isCreatingNewBrush) handleMouseMoveWhenCreating(event as any);
    if (movingBrush) handleOnDrag(event as any, currentBrushId);
    if (mouseDownForResize) handleOnResize(event as any, currentBrushId);
  }
