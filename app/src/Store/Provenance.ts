    ...
    graphRTD
      ...
      .then(function(dataSnapshot) {
        ...
        for (let j in dataJson.nodes) {
          ...
          // Type this to help you not miss anything.
          const node: ProvenanceNode<IntentState, any, any> = dataJson.nodes[j];
          const { state } = node;

          let { plots = [] } = state;

          // This converts the plots to a dictionary.
          plots = convertObjToArray(plots).map((d) => ({
            ...getDefaultPlot(),
            ...d,
          }));
          console.log(plots);
