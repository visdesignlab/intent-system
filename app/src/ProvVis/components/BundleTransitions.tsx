...
export default function bundleTransitions(xOffset: number, yOffset: number, clusterOffset: number, backboneOffset: number, duration: number = 500, stratifiedMap: any, nodeList: any[], annotationOpen: number, annotationHeight: number, bundleMap?: BundleMap) {
    xOffset = -xOffset;
    ...
    const enter = (data: any) => {
        ...
        if (bundleMap) {
            ...
            for (let i = 0; i < nodeList.length; i++) {
                if (node.width === 0 && nodeList[i].width === 0 && nodeList[i].depth <= highestDepth && backboneBundleNodes.includes(nodeList[i].id)) {
